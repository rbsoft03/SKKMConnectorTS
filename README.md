...
Библиотека `RBSoftSkkm` — программная обёртка над REST API **Сервера ККМ** для приложений на TypeScript/JavaScript (Node.js и браузер).

Позволяет добавить печать чеков и управление ККТ в любое TS/JS-приложение без ручной сборки HTTP-запросов и JSON. Работа строится вокруг одного класса `SkkmConnector`: заполняете свойства, вызываете метод, читаете результат из свойств .
...
- один экземпляр `SkkmConnector` не рассчитан на параллельные запросы к разным кассам одновременно — на параллельную работу создавайте отдельные экземпляры;
...
Обычный способ поставки клиенту. Пакет `RBSoftSkkm` ставится из локального архива, собранного через `npm pack` (в репозитории коннектора после сборки появляется `RBSoftSkkm-1.0.0.tgz`).

```bash
npm install /путь/до/коннектора/RBSoftSkkm-1.0.0.tgz
```
...
```json
"dependencies": {
  "RBSoftSkkm": "file:../RBSoftSkkm-1.0.0.tgz"
}
```
